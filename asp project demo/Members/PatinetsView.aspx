...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:GridView ID="GridView1" runat="server"  AllowPaging="True" style="width:fit-content" AutoGenerateColumns="False" BackColor="#DEBA84" BorderColor="#DEBA84" BorderStyle="None" BorderWidth="1px" CellPadding="3" CellSpacing="2" DataKeyNames="Pid" DataSourceID="SqlDataSource1" OnRowUpdating="GridView1_RowUpdating" PageSize="5">
        <Columns>
            ...
            <asp:TemplateField HeaderText="Category" SortExpression="category">
                <EditItemTemplate>
                    <%-- <asp:TextBox ID="TextBox2" Text='<%# Bind("category") %>' runat="server"></asp:TextBox>--%>
                    
                    <asp:DropDownList ID="ddlctg" DataSourceID="SqlDataSource2" runat="server" AutoPostBack="True" DataTextField="category" DataValueField="ctid" SelectedValue='<%# Bind("category") %>'></asp:DropDownList>
            ...
            <asp:CheckBoxField DataField="Isadmitted" HeaderText="Isadmitted" SortExpression="Isadmitted" />
            <asp:TemplateField HeaderText="CId" SortExpression="ctid">
                <EditItemTemplate>
                    <asp:TextBox ID="TextBox5" runat="server" Text='<%# Bind("ctid") %>'></asp:TextBox>
                    <asp:RequiredFieldValidator ID="RequiredFieldValidator1" runat="server" ControlToValidate="TextBox5" ErrorMessage=" ctid Field Required"></asp:RequiredFieldValidator>
                </EditItemTemplate>
                <ItemTemplate>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:hmdbCon %>" DeleteCommand="DELETE FROM [patient] WHERE [Pid] = @Pid" InsertCommand="INSERT INTO [patient] ([Pid], [name], [email], [date], [category], [Picture], [Isadmitted], [ctid]) VALUES (@Pid, @name, @email, @date, @category, @Picture, @Isadmitted, @ctid)" SelectCommand="select c.category,* from patient p,categories c where p.ctid=c.ctid" UpdateCommand="UPDATE [patient] SET [name] = @name, [email] = @email, [date] = @date, [category] = @category, [Picture] = @Picture, [Isadmitted] = @Isadmitted, [ctid] = @ctid WHERE [Pid] = @Pid">
        <DeleteParameters>
